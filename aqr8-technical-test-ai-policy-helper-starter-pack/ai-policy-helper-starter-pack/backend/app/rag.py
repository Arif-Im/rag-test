...
class StubLLM:
    def generate(self, query: str, contexts: List[Dict]) -> str:
        if not contexts:
            return "I'm sorry, I couldn't find any relevant documents to answer that question."
        
        sources_list = [f"- {c.get('title')} ({c.get('section', 'N/A')})" for c in contexts]
        sources_text = "\n".join(sources_list)
        
        return (
            f"STUB MODE: I found {len(contexts)} relevant pieces of information.\n\n"
            f"Sources identified:\n{sources_text}\n\n"
            f"Top Match Preview: {contexts[0].get('text', '')[:200]}..."
        )
    ...
    def __init__(self, api_key: str, model: str = "openai/gpt-4o-mini"):
        ...
        self.client = OpenAI(
            ...
            default_headers={
                ...
                "X-Title": "AI Policy Helper",
            }
        )
        self.model = model

    def generate(self, query: str, contexts: List[Dict]) -> str:
        context_text = "\n".join([c.get('text', '')[:200] for c in contexts])
...
class RAGEngine:
    def __init__(self):
        ...
        print(f"DEBUG: Provider from settings: '{settings.llm_provider}'")
        print(f"DEBUG: API Key present: {bool(settings.openrouter_api_key)}")
